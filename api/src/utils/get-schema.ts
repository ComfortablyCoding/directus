...
import type { CollectionMeta, FieldMeta, Filter, SchemaOverview } from '@directus/types';
...
async function getDatabaseSchema(database: Knex, schemaInspector: SchemaInspector): Promise<SchemaOverview> {
	...
	for (const [collection, info] of Object.entries(schemaOverview)) {
		...
		result.collections[collection] = {
			...
			versionOf: collectionMeta?.versionOf ?? null,
			versionedBy: collectionMeta?.versionedBy ?? null,
		...
		...(await database
			.select<
				Pick<
					...
					| 'versionedBy'
				>[]
			>('id', 'collection', 'field', 'special', 'note', 'validation', 'searchable', 'system', 'versionOf', 'versionedBy')
	...
	for (const field of fields) {
		...
		result.collections[field.collection]!.fields[field.field] = {
			...
			versionOf: field.versionOf ?? null,
			versionedBy: field.versionedBy ?? null,
		};
	}
