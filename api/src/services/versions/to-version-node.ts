...
export function toVersionNode<T extends M2ONode>(node: T): T {
	...
	if (child.type === 'm2o') {
		child.fieldKey = 'shadow_' + child.fieldKey;
		child.name = 'shadow_' + child.name;
		child.relation.related_collection = 'shadow_' + child.relation.related_collection!;
		child.relation.field = 'shadow_' + child.relation.field;

		if (child.relation.schema) {
			child.relation.schema.foreign_key_table &&= 'shadow_' + child.relation.schema?.foreign_key_table;
			child.relation.schema.foreign_key_column &&= VERSION_SYSTEM_FIELDS.primary.field;
		}

		if (child.relation.meta) {
			child.relation.meta.many_field &&= 'shadow_' + child.relation.meta.many_field;
			child.relation.meta.one_collection &&= 'shadow_' + child.relation.meta.one_collection;
		}
	}
